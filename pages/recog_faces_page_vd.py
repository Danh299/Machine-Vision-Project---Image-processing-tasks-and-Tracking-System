    ...
    with pn1:
        try:      
            if vd_file is not None:
                ...
                st.video(vd_bytes)
                output_path = "videos/out_faces1.mp4"         # Create an output file .mp4 for processing and showing
    ...
    with pn2:
        if converted:
            with st.spinner("Processing video..."):
                success = Recog_Video_Face(os.path.join("videos", vd_file.name), output_path)
