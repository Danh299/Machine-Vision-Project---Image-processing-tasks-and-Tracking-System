...
def ConnectedComponents(imgin):
    ...
    for x in range (0,M):
        for y in range (0,N):
            ...
            if r>0:
                a[r]=a[r]+1
    s='So thanh phan lien thong %d'%(dem-1)
    cv2.putText(imgout, s, (10, 15), cv2.FONT_HERSHEY_SIMPLEX,0.5,(255,255,255)) # in        
    for r in range (1, dem):
         s='%3d %5d'%(r,a[r])
         cv2.putText(imgout, s, (10, (r+1)*15), cv2.FONT_HERSHEY_SIMPLEX,0.5,(255,255,255)) # in  
    return imgout

def RemoveSmallRice(imgin):
    # Làm đậm bóng của hạt gạo dùng biến đổi top-hat
    # 81 là kích thước lớn nhất của hạt gạo (đo bằng pixel)
    w = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (81,81))
    temp = cv2.morphologyEx(imgin, cv2.MORPH_TOPHAT, w)
    nguong = 120
    _, imgout = cv2.threshold(temp, nguong, L-1, cv2.THRESH_BINARY|cv2.THRESH_OTSU)
    dem, label = cv2.connectedComponents(imgout, None)
    a = np.zeros(dem, np.int32)
    M, N = label.shape
    for x in range(0, M):
